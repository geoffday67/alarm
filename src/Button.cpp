...
void Button::onEvent(int type, void *pdata) {
    TouchEvent *pevent = (TouchEvent*) pdata;
    if (contains (pevent->x, pevent->y)) {
        if (pevent->down) {
            showDown();
            EventManager.fireEvent(EVENT_BUTTON, new ButtonEvent(_id));
        } else {
            showUp();
        }        
    }
